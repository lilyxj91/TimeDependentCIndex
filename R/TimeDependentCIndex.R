#' Time-dependent C-index for recurrent events
#'
#' @param train.data  training data set
#' @param validate.dataInput validation data set
#' @param n.var Number of predictors.
...
TimeDependentCIndex <- function(train.data,
  ...
  for (ii in 1:B) {
    data.risk$w <- rep(rexp(n), table(data.risk$id))
...
#' Recurrent event regression (proportional means model)
#'
#' @param data data set
#' @param beta1 coefficient
#' @param X
#'
#' @return par2 A list with optimization results
#' @export
my.reReg <- function(data, beta1, X) {
  data0j <- data %>%
    group_by(id) %>%
    slice(1) %>%
    dplyr::select(-c("indi", "t"))
  data0i <- data %>%
    ungroup() %>%
    filter(Num != 0) %>%
    dplyr::select(-c("indi", "t.obs"))
...
#' @param X
#'
#' @return par2.pb A list with optimization results
#' @export
...
#' Objective functions: use estimating equation
#'
...
#' Objective functions: use estimating equation with pb
#'
#' @param beta coefficient
#' @param input_list list of inputs
#'
...
#' Objective functions: use mimic log partial likelihood
#'
#' @param beta coefficient
#' @param input_list list of inputs
#'
...
#' Objective functions: use mimic log partial likelihood pb
#'
#' @param beta coefficient
#' @param input_list list of inputs
#'
...
#' calculate the counts of concordance for estimation
#'
#' @param ss time
#' @param data.risk data
#'
#' @return list of two counts
#' @export
My.counts <- function(ss, data.risk) {
  ...
  data.risk.agg <- data.risk %>%
    group_by(id) %>%
    mutate(N.counts = sum(indicator))
...
#' calculate the counts of concordance for variance estimation: using perturbation
#'
#' @param ss time
#' @param data.risk data
#'
